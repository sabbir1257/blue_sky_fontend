...
import { FacebookIcon, LinkedinIcon, MenuIcon, TwitterIcon, YoutubeIcon } from '@/components/Icons/Icons'
    ...
    return (
        <div className='lg:max-w-[1200px] mx-auto lg:px-4 py-2'>
            ...
            <nav className='text-white bg-blue-900 px-5 md:px-10 md:py-5 py-4 rounded-sm flex justify-between relative z-[999]'>
                    ...
                    <ul className="text-gray-300 flex gap-6 text-lg">
                        {navLinks?.map((link: TNavLinks, i: number) => {
                            ...
                            return (
                                <li
                                    key={i}
                                    className={`group relative ${isActive ? "text-white font-medium" : ""}`}
                                >
                                    <Link href={link?.path} className="relative inline-block uppercase">
                                        {link?.name}
                                        <span
                                    ...
                                    {link?.children && (
                                        <div className="absolute left-0 top-full mt-2 bg-blue-800 text-white rounded-md shadow-lg w-72 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 ease-in-out">
                                            {link.children.map((item: any, j: number) => (
                                                <div key={j}>
                                                    <Link href={item.path} className="block px-4 py-2 hover:bg-blue-700">
                                                        {item.name}
                                                    </Link>
                                                </div>
                                            ))}
                ...
                {/* Mobile Menu */}
                <div className='md:hidden text-2xl' onClick={() => setOpen(prev => !prev)}>
                    <MenuIcon />
                </div>

                <div className={`absolute top-0 left-0 bg-blue-900 w-full h-screen transition-transform duration-300 md:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}>
