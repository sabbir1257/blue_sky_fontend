...
const TotalAnalysis = () => {
  ...
  const targetValues = [30, 100, 53_000, 24]; // Target numbers
            ...
            setCounts((prevCounts) =>
              prevCounts.map((count, index) =>
                count < targetValues[index]
                  ? Math.min(count + Math.ceil(targetValues[index] / 100), targetValues[index])
                  : count
              )
            );
          }, 20); // Speed of increment

          setTimeout(() => clearInterval(interval), 2000); // Stop incrementing after 2 seconds
        }
      },
      { threshold: 0.5 }
    );
